...
async def chat_completions(request: ChatCompletionRequest, http_req: Request):
    ...
    _preview = ""
    try:
        _last = next((m for m in reversed(request.messages) if getattr(m, "role", None) == "user"), None)
        _c = getattr(_last, "content", None) if _last else None
        if isinstance(_c, str):
            _preview = _c
        elif isinstance(_c, list):
            _preview = " ".join(p.get("text", "") for p in _c if isinstance(p, dict))
        _preview = (_preview or "").replace(chr(10), " ").strip()[:1500]
    except Exception:
        pass
    logger.info(f"[completions] req_id={req_id} 수신: model={request.model} stream={request.stream} | 입력({len(_preview)}자): {_preview}")
        ...
        async for raw_chunk in client.infer_stream_raw(**vllm_params):
            ...
            if raw_chunk is not None:
                ...
                if delta:
                    ...
                    if not first_punct_logged and any(p in delta for p in ".,?!。，"):
                        first_punct_ms = (now - t0) * 1000
                        first_punct_logged = True
            if raw_chunk and "usage" in raw_chunk:
                prompt_tokens = raw_chunk["usage"].get("prompt_tokens", 0)
                completion_tokens = raw_chunk["usage"].get("completion_tokens", 0)
    ...
    itl_ms = (decode_ms / (chunk_count - 1)) if chunk_count > 1 else 0.0
    response_text = "".join(full_response_parts).replace("\n", " ").strip()
    raw_response = {"choices": [{"message": {"role": "assistant", "content": response_text}, "finish_reason": "stop", "index": 0}], "usage": {"prompt_tokens": prompt_tokens, "completion_tokens": completion_tokens, "total_tokens": prompt_tokens + completion_tokens}, "model": request.model}
    res = response_converter.convert_completion(raw_response, request.model, None)
    logger.info(f"[completions] req_id={req_id} {request.model} 완료: {latency_ms:.0f}ms | TTFT: {ttft_ms:.0f}ms | ITL: {itl_ms:.1f}ms | punct: {first_punct_ms:.0f}ms | chunks: {chunk_count} | prompt: {prompt_tokens}tok | completion: {completion_tokens}tok | 응답: {response_text[:200]}")
