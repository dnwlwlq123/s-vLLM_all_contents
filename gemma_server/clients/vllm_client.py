...
class VLLMClient(BaseClient):
    ...
    async def infer_raw(
        ...
        **kwargs,
        ...
        url = f"{self.base_url}/v1/chat/completions"
        client = await self._get_client()
        headers = {"x-request-id": kwargs["_request_id"]} if kwargs.get("_request_id") else None
    ...
    def _build_body(
        ...
        **kwargs,
        ...
        # CE-Service에서 넘어온 추가 파라미터 + gateway 가 주입하는 vLLM 확장 필드
        for key in ("top_k", "presence_penalty", "response_format", "guided_choice"):
