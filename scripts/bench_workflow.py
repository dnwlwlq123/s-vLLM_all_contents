#!/usr/bin/env python3
"""
KB chatbot example 부하테스트 v3 — TPOT 정확 계산 + per-call timeout
...
RAG_CHUNKS = [
    ...
    "[문의처] KBExample Bank 콜센터 1599-9999 / 영업시간 평일 9~17시 / 주말 미운영. ",
...
def build_prompt(tok, target_tokens: int, channel_id: int, turn: int, step_idx: int) -> str:
    """재구성형(B) 워크플로우 — step별 system prompt 다름, prefix 공유 없음.
    매 호출마다 step_idx 포함한 시드로 다른 RAG 셔플 → 첫 토큰부터 다른 시퀀스 → cache hit 0%.
    O(N) — encode 1회.
    """
    rng = random.Random(channel_id * 10000 + turn * 100 + step_idx)
    pool = RAG_CHUNKS[:]
    rng.shuffle(pool)
    repeats = max(2, target_tokens // 100 + 5)
    text = "\n".join(pool * repeats)
