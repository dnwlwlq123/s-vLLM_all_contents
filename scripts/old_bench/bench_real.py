...
TOK_PATH = '/workspace/models/gemma-4-31B-it'
...
RAG_TEMPLATES = [
    '[상품 정보]\n상품명: {name}\n금리: 연 {rate}% / 세후 {rate2}%\n가입기간: {period}개월\n최소금액: {amount}만원\n세금: 이자소득세 15.4% 원천징수\n중도해지: 중도해지 시 기본금리 적용\n가입조건: 만 {age}세 이상 개인, 비대면 가입 가능\n상세약관: 이 상품은 Example Bank 고객을 위한 특별 우대 상품으로 자동이체 연계 시 추가 금리 혜택이 있으며 청소년, 군인, 주택청약 연계 고객은 별도 우대금리가 적용됩니다.\n' * 3,
...
async def send(sess, i, delay, results):
    ...
    body = {'model':MODEL,'messages':[{'role':'system','content':SYSTEM},{'role':'user','content':user}],
            'max_tokens':OUTPUT_TOKS,'temperature':0.3,'stream':True,'stream_options':{'include_usage':True}}
